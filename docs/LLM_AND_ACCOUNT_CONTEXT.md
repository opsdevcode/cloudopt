...
| **RAG (`rag_chunks`)** | Embeddings over findings text, **scan summaries** (after each finops run), and later CUR snippets—scoped by **`tenant_id`**. Optional audit embedding via `CLOUDOPT_RAG_EMBED_AUDIT` (top-N by severity). API: `GET /api/v1/rag/search`, `POST /api/v1/rag/ask`. |
